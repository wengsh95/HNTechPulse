...
def generate_fixed_opening(
    ...
    The opening is intentionally deterministic. It only needs to introduce
    the already-selected story hooks, while the substantive narration is
    generated by the story-scan calls below. Keeping this local avoids a
    low-value one-line LLM round trip and makes script regeneration stable.
    ...
    for entry in entries[:2]:
        hook = _entry_spoken_hook(entry, max_len=10)
        if hook and hook != "技术信号" and hook not in spoken_hooks:
            spoken_hooks.append(hook)

    if spoken_hooks:
        hook_text = "，再看".join(spoken_hooks)
        audio_text = f"今天的HN观察，先看{hook_text}。"
    else:
