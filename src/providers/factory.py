...
def _auto_register():
    ...
    attempts = [
        ...
        ("tts", "openai", "src.providers.tts.openai_tts", "OpenAITTSProvider", register_tts),
    ...
    for kind, name, module_path, cls_name, register_fn in attempts:
        try:
            ...
            _logger.warning(f"{kind} provider '{name}' unavailable ({module_path}): {e}")
        except AttributeError as e:
            _logger.error(f"{kind} provider '{name}': {cls_name} not found in {module_path}: {e}")
