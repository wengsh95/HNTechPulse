...
- **1-3 个立场标签** (支持/质疑/中立), value 为 0-1, 总和为 1. **不强制 3 个**:
  - 强对立故事 → 输出支持/质疑 2 立场
  - 一边倒的故事 → 只输出 1 个主要立场 (如只有"支持")
  - 平淡/信息补充型 → 只输出"中立" 1 立场
- 根据评论的 sentiment 和内容判断，不要机械地按数量均分。
- **禁止**为"假平衡"硬造对立的质疑方.
- 如果 discussion_mode 不是 debate, 默认只有 1 个主要立场 (通常是"中立").

stance_concerns（立场关切）:
- **只对 stance_distribution 中实际出现的立场输出 concern**. 1 个立场就 1 个, 3 个就 3 个, 不要凑数.
- 每个 concern 用 6-12 个字概括该立场群体最关心的核心问题.
- 多立场时, concerns 必须与 debate_focus 对应, 体现分歧根源.
- 1 个立场时, 写 1 条关切即可, 不需要"另一方".
- 例子:
  - 多立场: 支持方"隐私权优先", 质疑方"安全更新风险"
  - 1 立场: 中立"权衡因人而异" 或 信息补充型"细节澄清为主"
