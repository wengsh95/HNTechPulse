...
def main() -> None:
    ...
    parser.add_argument(
        "--skip-render", action="store_true", help="Skip the render step (prepare only)"
    )
    parser.add_argument(
        "--skip-review", action="store_true", help="Skip the review frame extraction"
    )
    args = parser.parse_args()

    date = args.date
    render_script = str(
        PROJECT_ROOT / "scripts" / "render_hyperframes_from_cli_props.py"
    )
